...
def strategy(assets):
    ...
    for i in range(len(assets)):
        df= web.DataReader(assets[i], 'yahoo','2010-01-01')
        ...
        print 'Total money returned by the HFT backtest is ',portfolio.iloc[-1]['total'] 
        print portfolio
        ...
        plt.title('Gold ETF showing Drawdowns')
        plt.ylabel('Amount')
        plt.xlabel('Date')
...
plt.xlabel('Date')
plt.plot([i, j], [portfolio['total'][i], portfolio['total'][j]], 'o', color='Red', markersize=10)
plt.plot([i, j], [portfolio['total'][i], portfolio['total'][j]], 'g', color='Red', markersize=10)
